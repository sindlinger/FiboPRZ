...
// --- ZigZag primário (fonte de pivôs)
...
// --- ZigZag secundário (visual independente)
...
// --- Escopo de escaneamento (apenas ZigZag)
...
// --- Janela global (centrada no preço atual)
input bool     InpUseGlobalWindow        = false;  // limita linhas à janela global?
input double   InpGlobalWindowPctATR     = 300.0;  // largura = % do ATR(D1)
input double   InpGlobalWindowRangeMultiplier = 1.0;  // mínimo = múltiplos do range D1
input double   InpGlobalWindowWidthDays  = 1.0;   // largura horizontal = múltiplos de 1 dia
input int      InpGlobalWindowCenterMAPeriod = 0; // 0 = meio do candle, >0 = MA(D1) para centro
input bool     InpShowGlobalWindowInfo   = false; // desenhar texto com dados da janela?
input int      InpGlobalWindowExtendBars = 12;    // extensão horizontal extra (em barras)
input bool     InpDebugDrawGlobalBand    = false;  // desenhar faixa global?
input color    InpGlobalBandColor        = clrDimGray;
...
// --- Debug focal (últimos N traçados; 0 = OFF)
...
enum ENUM_GW_FALLBACK_FLAG { GW_FALLBACK_NONE=0, GW_FALLBACK_PRICE=1, GW_FALLBACK_ATR=2 };
// PRZ (opcional)
...
string          G_PREF_GW_BAND = "FCZ_GW_BAND";
string          G_PREF_GW_WARN = "FCZ_GW_WARN";
string          G_PREF_GW_INFO = "FCZ_GW_INFO";
...
bool GetGlobalWindowBounds(const datetime &time[], const double &high[], const double &low[],
                           int total,
...
{
   fallback_flags = GW_FALLBACK_NONE;
   ...
   if(!haveIntraday || dayHigh<=dayLow){
      ...
      fallback_flags |= GW_FALLBACK_PRICE;
   }
   ...
   if(!GetATR_D1(InpATR_D1_Periods, atrD1) || atrD1<=0.0){
      atrD1 = baseRange;
      fallback_flags |= GW_FALLBACK_ATR;
   }

   double center = (dayHigh + dayLow) * 0.5;
   int centerPeriod = MathMax(0, InpGlobalWindowCenterMAPeriod);
   if(centerPeriod>0){
   ...
   double pct = MathMax(0.01, InpGlobalWindowPctATR);
   double spanAtr = atrD1 * (pct/100.0);
   double mult = MathMin(5.0, MathMax(1.0, InpGlobalWindowRangeMultiplier));
   ...
   double widthDays = MathMax(0.1, InpGlobalWindowWidthDays);
   ...
   datetime midTime = (datetime)((long)dayStart + (long)(MathMax(1, daySeconds)/2));
   datetime baseStart = (datetime)((long)midTime - halfWidth);
   datetime baseEnd   = (datetime)((long)midTime + halfWidth);
   int extendBars = MathMax(0, InpGlobalWindowExtendBars);
...
void ShowGlobalWindowFallbackNotice(int flags)
{
   const string name = G_PREF_GW_WARN;
   if(flags==GW_FALLBACK_NONE){
      ObjectDelete(ChartID(), name);
      return;
   }
   string parts="";
   if((flags & GW_FALLBACK_PRICE)!=0) parts = "Hi/Lo intraday";
   if((flags & GW_FALLBACK_ATR)!=0){
      if(parts!="") parts += " + ";
   ...
   ObjectSetString(ChartID(), name, OBJPROP_TEXT, "Aviso: janela global fallback ("+parts+")");
}
void ShowGlobalWindowInfo(bool enabled,bool hasData,double low,double high,double mid,double atr,int flags,bool centerFromMA)
{
   const string name = G_PREF_GW_INFO;
   if(!enabled){
      ObjectDelete(ChartID(), name);
      return;
   }
   string text;
   if(!hasData){
      text = "Janela global: dados indisponíveis.";
   }else{
      string fonte = "ATR% + Range";
      if((flags & GW_FALLBACK_ATR)!=0) fonte = "RangeOnly";
      if((flags & GW_FALLBACK_PRICE)!=0) fonte += "+Fallback";
      string centerText = (centerFromMA && InpGlobalWindowCenterMAPeriod>0?
                           StringFormat("MA(D1,%d)", InpGlobalWindowCenterMAPeriod) :
                           "Mid Hi/Lo D1");
      double span = high - low;
      text = StringFormat("Janela Global: [%.5f .. %.5f] span=%.5f\nCentro=%s %.5f  Fonte=%s\nATR(1D,p=%d)=%.5f  RangeMult=%.2f  Largura=%.2f D1",
                          low, high, span,
                          centerText, mid, fonte,
                          InpATR_D1_Periods, atr,
                          MathMax(1.0, MathMin(5.0, InpGlobalWindowRangeMultiplier)),
                          MathMax(0.1, InpGlobalWindowWidthDays));
   }
   if(ObjectFind(ChartID(), name)<0) ObjectCreate(ChartID(), name, OBJ_LABEL, 0, 0, 0);
   ObjectSetInteger(ChartID(), name, OBJPROP_CORNER, CORNER_LEFT_UPPER);
   ObjectSetInteger(ChartID(), name, OBJPROP_XDISTANCE, 6);
...
void DrawGlobalWindowBand(bool enabled,double low,double high,datetime t_start,datetime t_end,const datetime &time[],int total)
{
   const string name = G_PREF_GW_BAND;
   if(!enabled || low>=high){
      ObjectDelete(ChartID(), name);
      return;
   }
   ...
   if(t_start!=0 && t_end!=0 && t_start!=t_end){
      tL = (t_start<t_end? t_start : t_end);
      tR = (t_start<t_end? t_end : t_start);
   }else if(!GetVisibleTimeRange(time,total,tL,tR)){
      bool series=IsSeries(time,total);
      tL = time[series? total-1 : 0];
      tR = time[series? 0 : total-1];
   }
   if(tL==tR){
      int ps=PeriodSeconds(); if(ps<=0) ps=60;
      tR = (datetime)((long)tR + ps);
   }
   long cid=ChartID();
   if(ObjectFind(cid,name)<0) ObjectCreate(cid,name,OBJ_RECTANGLE,0,tL,high,tR,low);
   else{
      ObjectMove(cid,name,0,tL,high);
      ObjectMove(cid,name,1,tR,low);
   }
   ObjectSetInteger(cid,name,OBJPROP_BACK,true);
   ObjectSetInteger(cid,name,OBJPROP_COLOR,InpGlobalBandColor);
   ObjectSetInteger(cid,name,OBJPROP_WIDTH,1);
   ObjectSetInteger(cid,name,OBJPROP_FILL,false);
   ObjectSetInteger(cid,name,OBJPROP_SELECTABLE,false);
}
                          ...
                          const string &prefLine, const string &prefLabel, int &prevCount)
{
   ...
   for(int i=0;i<total && drawn<limit;i++)
   {
      ...
      UpsertHLineStyled(nm, source[i].price, lineColor, MathMax(1, InpFibLineWidth), STYLE_DASHDOTDOT);
                ...
                const int &spread[])
{
   ...
   bool needGW = (InpUseGlobalWindow || InpShowGlobalWindowInfo || InpDebugDrawGlobalBand);
   double gwLow=0.0, gwHigh=0.0, gwMid=0.0, gwAtr=0.0;
   datetime gwTStart=0, gwTEnd=0;
   int gwFlags = GW_FALLBACK_NONE;
   bool gwCenterMA=false;
   bool gwOk=false;
   if(needGW){
      gwOk = GetGlobalWindowBounds(time, high, low, rates_total,
                                   gwLow, gwHigh, gwMid, gwAtr, gwFlags,
                                   gwTStart, gwTEnd, gwCenterMA);
      ShowGlobalWindowFallbackNotice(gwFlags);
   }else{
      ShowGlobalWindowFallbackNotice(GW_FALLBACK_NONE);
   }

   bool applyGW = (gwOk && InpUseGlobalWindow);
   if(applyGW){
      ApplyDailyWindowFilter(g_all, g_view_price, gwLow, gwHigh);
   }

   DrawGlobalWindowBand((InpDebugDrawGlobalBand && gwOk), gwLow, gwHigh, gwTStart, gwTEnd, time, rates_total);
   ShowGlobalWindowInfo(InpShowGlobalWindowInfo, gwOk, gwLow, gwHigh, gwMid, gwAtr, gwFlags, gwCenterMA);
