...
input double   InpDailyWindowHeightPoints = 0.0;   // >0: altura absoluta (pontos); 0 = usar ATR/range
input int      InpDailyWindowWidthMinutes = 0;     // >0: largura absoluta (minutos); 0 = usar dias
...
DailyWindow ComputeDailyWindow(const datetime &time[], const double &high[], const double &low[], int total)
{
   DailyWindow dw;
   dw.valid=false;
   dw.low=dw.high=dw.mid=dw.atr=dw.span=0.0;
   dw.t_start=dw.t_end=0;
   dw.center_from_ma=false;
   dw.used_abs_height=false;
   dw.used_abs_width=false;
   dw.fallback_flags = DW_FALLBACK_NONE;

   if(total<=0) return dw;
   ...
   if(!haveIntraday || dayHigh<=dayLow){
      ...
      if(!haveDaily) return dw;
      dw.fallback_flags |= DW_FALLBACK_PRICE;
   }
   ...
   if(!GetATR_D1(InpATR_D1_Periods, atrD1) || atrD1<=0.0){
      atrD1 = baseRange;
      dw.fallback_flags |= DW_FALLBACK_ATR;
   }
   dw.atr = atrD1;
   ...
   if(centerPeriod>0){
      ...
      if(maHandle!=INVALID_HANDLE){
         ...
         if(copied>0 && MathIsValidNumber(buf[0])){
            center = buf[0];
            dw.center_from_ma = true;
         }
      }
   }

   double span=0.0;
   double absPts = MathMax(0.0, InpDailyWindowHeightPoints);
   if(absPts>0.0){
      double converted = absPts * _Point;
      span = (converted>0.0? converted : absPts);
      dw.used_abs_height = true;
   }
   if(span<=0.0){
   ...
   long widthSeconds=0;
   int absMinutes = MathMax(0, InpDailyWindowWidthMinutes);
   if(absMinutes>0){
      widthSeconds = (long)absMinutes * 60;
      dw.used_abs_width = true;
   }else{
   ...
   dw.t_start = (datetime)((long)baseStart - ext);
   dw.t_end   = (datetime)((long)baseEnd   + ext);

   dw.valid = (dw.low < dw.high);
   return dw;
}
...
void ShowDailyWindowInfo(bool enabled, const DailyWindow &dw)
{
   ...
   string fonte = (dw.used_abs_height? "AlturaAbs" : "ATR% + Range");
   if((dw.fallback_flags & DW_FALLBACK_ATR)!=0) fonte = "RangeOnly";
   if((dw.fallback_flags & DW_FALLBACK_PRICE)!=0) fonte += "+Fallback";
   if((dw.fallback_flags & DW_FALLBACK_EMPTY)!=0) fonte += "+SemLinhas";
   double altura = MathMax(0.0, dw.high-dw.low);
   double percReal = (dw.atr>0.0 ? (altura/dw.atr)*100.0 : 0.0);
   double percCfg  = MathMax(0.0, InpDailyWindowHeightPctATR);
   string realText = (dw.atr>0.0 ? StringFormat("%.1f%% ATR", percReal) : "n/d");
   string cfgText  = (dw.used_abs_height?
                      StringFormat("AlturaAbs=%.1f pts", MathMax(0.0, InpDailyWindowHeightPoints)) :
                      StringFormat("AltCfg=%.1f%% ATR  >=%.1fx Range", percCfg,
                                   MathMax(1.0, MathMin(5.0, InpDailyWindowRangeMultiplier))));
   string centerText = (dw.center_from_ma && InpDailyWindowCenterMAPeriod>0?
                        StringFormat("Centro=MA(%d)", InpDailyWindowCenterMAPeriod) :
                        "Centro=mid D1");
   string widthText = (dw.used_abs_width?
                       StringFormat("LargAbs=%d min (%.2f h)", MathMax(0, InpDailyWindowWidthMinutes),
                                    (double)MathMax(0, InpDailyWindowWidthMinutes)/60.0) :
                       StringFormat("Larg=%.2f D1", MathMax(0.1, InpDailyWindowWidthDays)));
   string text = StringFormat(
      "Janela D1  Low:%.5f  High:%.5f  Centro:%.5f  Altura:%.5f  Real:%s  %s  %s  Span:%.5f  ATR usado:%.5f (p=%d)  Fonte:%s",
      dw.low, dw.high, dw.mid, altura, realText, cfgText, widthText, dw.span, dw.atr, InpATR_D1_Periods, fonte);

   if(InpDebugLog)
      Dbg(StringFormat("[DW] low=%.5f high=%.5f mid=%.5f alt=%.5f span=%.5f real=%.1f cfg=%.1f absHeight=%s widthAbs=%s widthDays=%.2f atr=%.5f centroMA=%s fonte=%s",
                       dw.low, dw.high, dw.mid, altura, dw.span, percReal, percCfg,
                       (dw.used_abs_height? "SIM":"NAO"), (dw.used_abs_width? "SIM":"NAO"),
                       MathMax(0.1, InpDailyWindowWidthDays), dw.atr,
                       (dw.center_from_ma? "SIM" : "NAO"), fonte));
   if(ObjectFind(ChartID(), name)<0) ObjectCreate(ChartID(), name, OBJ_LABEL, 0, 0, 0);
   ObjectSetInteger(ChartID(), name, OBJPROP_CORNER, CORNER_LEFT_UPPER);
   ObjectSetInteger(ChartID(), name, OBJPROP_XDISTANCE, 6);
   ObjectSetInteger(ChartID(), name, OBJPROP_YDISTANCE, 6 + InpSummaryFontSize*5);
                ...
                const int &spread[])
{
   ...
   DailyWindow dw = ComputeDailyWindow(time, high, low, rates_total);
   if(!needDailyWindow) dw.fallback_flags = DW_FALLBACK_NONE;

   bool applyDailyWindow = (InpUseDailyWindow && dw.valid);
   g_dw_active = applyDailyWindow;
   g_dw_time_left  = (applyDailyWindow ? dw.t_start : 0);
   g_dw_time_right = (applyDailyWindow ? dw.t_end   : 0);

   if(applyDailyWindow){
      int backup[];
      ArrayCopy(backup, g_view_price);
      ApplyDailyWindowFilter(g_all, g_view_price, dw.low, dw.high);
      if(ArraySize(g_view_price)==0){
         ArrayCopy(g_view_price, backup);
         dw.fallback_flags |= DW_FALLBACK_EMPTY;
      }
   }

   ShowDailyWindowFallbackNotice(needDailyWindow? dw.fallback_flags : DW_FALLBACK_NONE);
   ShowDailyWindowInfo(InpShowDailyWindowInfo, dw);
