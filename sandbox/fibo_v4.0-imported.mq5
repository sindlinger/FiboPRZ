...
input int      InpZigZagPrimaryStartOffset   = 2;     // overlay: ignora X segmentos recentes
...
input int      InpZigZagSecondaryStartOffset = 2;     // overlay: ignora X segmentos recentes
...
// --- Pernas (linhas A->B)
input int      InpLegsToUse              = 26;      // quantas pernas usar
...
input double   InpClusterRangePctATR     = 4.0;   // ESPESSURA do cluster = % do ATR(1D)
input int      InpClusterMinLines        = 4;      // mínimo de linhas para existir cluster
...
void DrawGlobalWindowBand(bool enabled,double low,double high,const datetime &time[],int total)
{
   ...
   datetime tL=0,tR=0;
   if(!GetVisibleTimeRange(time,total,tL,tR)){
                ...
                const int &spread[])
{
   ...
   double gwLow=0.0, gwHigh=0.0;
   double gwCenter=0.0, gwHeight=0.0;
   bool gwApplied=false;
   if(InpUseGlobalWindow)
   {
      double atrGW=0.0;
      if(!GetATR_D1(InpATR_D1_Periods, atrGW) || atrGW<=0.0){
         double sumR=0.0; int cnt=MathMin(200, rates_total);
         for(int i=0;i<cnt;i++) sumR += (high[i]-low[i]);
         atrGW = (cnt>0? sumR/cnt : 0.0);
      }
      double width = atrGW * (InpGlobalWindowPctATR/100.0);
      if(width>0.0){
         double priceCenter=0.0;
         if(!SymbolInfoDouble(_Symbol, SYMBOL_BID, priceCenter) || !MathIsValidNumber(priceCenter) || priceCenter<=0.0){
            bool series=IsSeries(time,rates_total);
            int idx=series?0:rates_total-1;
            priceCenter = close[MathMax(0,MathMin(idx, rates_total-1))];
         }
         gwLow = priceCenter - width*0.5;
         gwHigh= priceCenter + width*0.5;
         gwCenter = priceCenter;
         gwHeight = width;
         if(gwLow < gwHigh){
   ...
   DrawGlobalWindowBand(InpDebugDrawGlobalBand && gwApplied, gwLow, gwHigh, time, rates_total);
