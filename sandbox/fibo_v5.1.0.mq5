...
input int      InpZigZagPrimaryStartOffset   = 1;     // ignora X segmentos recentes (cálculo/overlay)
...
input int      InpZigZagSecondaryStartOffset = 1;     // ignora X segmentos recentes (cálculo/overlay)
...
{
   if(g_price_digits>0)
      return g_price_digits;
   int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
   if(digits<=0)
      digits = (_Digits>0 ? _Digits : 5);
   g_price_digits = digits;
   return g_price_digits;
}

string FormatPrice(double value){ return DoubleToString(value, PriceDigits()); }
string FormatPercentValue(double value){ return TrimTrailingZeros(DoubleToString(value, 8)); }
string FormatRatioUnit(double ratio){ return TrimTrailingZeros(DoubleToString(ratio, 8)); }
string FormatRatioAsPercent(double ratio){ return TrimTrailingZeros(DoubleToString(ratio*100.0, 8)); }
string FormatGenericValue(double value, int digits)
{
...
string RatioTag(double ratio)
{
   ...
   return (is_exp? "X":"R") + FormatRatioAsPercent(ratio);
}
...
void ClearAllPriceLabels()
{
   ...
   Dbg(StringFormat("[Clear] price labels slots=%d objects=%d", ArraySize(g_label_slot_identity), cleared));
}
...
bool BuildPricePipelineResult(const double &high[], const double &low[], const datetime &time[],
                              int rates_total, int pivot_lookback, int trim_segments, int legs_to_use,
                              PricePipelineResult &result)
{
   result.Clear();
   return BuildPricePipeline(high, low, time, rates_total,
                             pivot_lookback, trim_segments, legs_to_use,
                             result.pivots, result.pivot_count,
                             result.legs, result.leg_count,
                             result.price_lines, result.price_count);
}
...
void ComputeClusterMembershipAndZones(const FibItem &all[],
                                      const double &P[], const int &idx_sorted[], int n,
                                      ...
                                      PRZ &zones[], int &zone_count)
{
   ...
   for(int anchor=0; anchor<bucketCount; ++anchor)
   {
      ...
      for(int bucket=s; bucket<e; ++bucket)
      {
         ...
         for(int k=0;k<len;k++){
            ...
            if(all_idx<0 || all_idx>=ArraySize(all)) continue;
            totalCnt++;
   ...
   if(!make_zones || ArraySize(cand)==0)
      return;
...
{
   int drawn=0;
   datetime labelLeft=0,labelRight=0;
   if(total>0){
      bool series = IsSeries(time,total);
      labelLeft  = time[series? total-1 : 0];
      labelRight = time[series? 0 : total-1];
      if(labelLeft>labelRight){
         datetime tmp=labelLeft;
         labelLeft=labelRight;
         labelRight=tmp;
      }
      int ps=PeriodSeconds(); if(ps<=0) ps=60;
      long margin = (long)ps * (long)MathMax(0, InpRightTextMarginBars);
      labelRight = (datetime)((long)labelRight + margin);
      if(labelLeft==labelRight){
         labelLeft = (datetime)((long)labelLeft - ps);
         labelRight = (datetime)((long)labelRight + ps);
      }
   }
   ...
   for(int k=0;k<n;k++){
      ...
      if(InpShowLabels){
         ...
         MaintainPriceLabels(all[i], slotIdx, identBase, priceToDraw, lbl, col, labelLeft, labelRight);
      }
      drawn++;
   }
...
void ClearDebugPriceObjects(const string &prefLine,const string &prefLabel,int &prevCount)
{
   ...
   Dbg(StringFormat("[Clear] dbg price pref_line=%s pref_label=%s count=%d", prefLine, prefLabel, prevCount));
   prevCount=0;
}
                          ...
                          const string &prefLine, const string &prefLabel, int &prevCount)
{
   ...
   if(rates_total>0){
      ...
      int ps=PeriodSeconds(); if(ps<=0) ps=60;
      long margin = (long)ps * (long)MathMax(0, InpRightTextMarginBars);
      labelTime = (datetime)((long)right + margin);
   }
                         ...
                         int &prevDots, int &prevVLines)
{
   ...
   for(int i=0;i<total && drawn<limit;i++)
   {
      ...
      string text = StringFormat("DBG T %s %s", FormatRatioUnit(source[i].ratio), (source[i].forward? "F" : "B"));
                ...
                const int &spread[])
{
   ...
   const bool series = IsSeries(time, rates_total);
   ...
   PricePipelineResult pricePipeline;
   pricePipeline.Clear();
   bool price_pipeline_ready = BuildPricePipelineResult(high, low, time, rates_total,
   ...
   g_leg_total = pricePipeline.leg_count;
   DrawLegs(pricePipeline.legs, pricePipeline.leg_count);

   ArrayResize(g_price_all, pricePipeline.price_count);
   ArrayCopy(g_price_all, pricePipeline.price_lines);
   g_price_total = pricePipeline.price_count;

   // 2) Linhas PREÇO + TEMPO
   if(InpShowTimeFibs){ BuildTimeMarks(pricePipeline.legs, pricePipeline.leg_count, g_time_all, g_time_total); }
   ...
   EnforceMaxLineLimit(g_view_price, g_all, maxPriceLines, InpMaxLineTrimMode, refPriceForTrim, refPriceValid);
   ...
   if(InpPriceMode==PRICE_RAW)
   {
      ...
      if(rates_total>0){
         bool series = IsSeries(time,rates_total);
         labelLeft  = time[series? rates_total-1 : 0];
         labelRight = time[series? 0 : rates_total-1];
         if(labelLeft>labelRight){
            datetime tmp=labelLeft;
            labelLeft=labelRight;
            labelRight=tmp;
      ...
      int lineWidth = MathMax(1, InpFibLineWidth);
        ...
        for(int k=0;k<ArraySize(g_view_price);k++){
            ...
            if(InpShowLabels){
               ...
               int slotIdx = LabelSlotAcquire(identBase);
               MaintainPriceLabels(g_all[i], slotIdx, identBase, priceToDraw, lbl, col, labelLeft, labelRight);
            }
            drawn++;
         }
      for(int i=drawn;i<g_prev_line_count;i++){
   ...
   else // PRICE_CLUSTER
   {
     ...
     ComputeClusterMembershipAndZones(g_all, P, idx_sorted, n,
                                      cluster_range, InpClusterMinLines,
                                      true, cluster_range,
                                      member_all, g_prz, g_prz_count);

     int maxClusterLines = MathMax(0, InpMaxClusterLines);
     if(maxClusterLines>0)
     {
        int clusterFibIdx[];
        for(int k=0;k<n;k++)
        {
           int fibIdx = g_view_price[k];
           if(fibIdx<0 || fibIdx>=g_all_total) continue;
           if(!member_all[fibIdx]) continue;
           int s = ArraySize(clusterFibIdx);
           ArrayResize(clusterFibIdx, s+1);
           clusterFibIdx[s] = fibIdx;
        }
        if(ArraySize(clusterFibIdx) > maxClusterLines)
        {
           EnforceMaxLineLimit(clusterFibIdx, g_all, maxClusterLines, InpMaxClusterLineTrimMode,
                               refPriceForTrim, refPriceValid);

           bool keep[]; ArrayResize(keep, g_all_total);
           for(int i=0;i<g_all_total;i++) keep[i]=false;
           for(int i=0;i<ArraySize(clusterFibIdx);i++)
           {
              int fibIdx = clusterFibIdx[i];
              if(fibIdx>=0 && fibIdx<g_all_total)
                 keep[fibIdx]=true;
           }
           for(int i=0;i<g_all_total;i++)
              member_all[i] = member_all[i] && keep[i];
        }
     }
     for(int k=0;k<n;k++){
     ...
     Dbg(StringFormat("[Fibo][%s] Src=ZZ  ATR(1D,p=%d)=%s  Range=%s%%  MinLines=%d  PRZ=%d  ClusterLines=%d  LinesTot=%d",
           _Symbol,
            InpATR_D1_Periods, FormatPrice(atrD1), FormatPercentValue(InpClusterRangePctATR),
            InpClusterMinLines,
   ...
   if(InpShowSummary)
   {
      string ln1 = StringFormat(
         ...
         ArraySize(g_view_price), g_visible_cluster_lines, g_prz_count,
         FormatPercentValue(InpClusterRangePctATR), InpATR_D1_Periods
      );
